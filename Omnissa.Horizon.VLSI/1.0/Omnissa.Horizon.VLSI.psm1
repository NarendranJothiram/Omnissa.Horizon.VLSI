...
# Example: Invoke-GetNextQuery -sessionInfo $sessionInfo -payload $payload -allResponses ([ref]$allResponses)
function Invoke-GetNextQuery {
    param (
        [Parameter(Mandatory = $true)]
        $sessionInfo,
        [Parameter(Mandatory = $true)]
    ...
    try {
        ...
        $response = Invoke-HorizonPostMethod $sessionInfo "queryservice/getnext" $payload

        # Add the response to the collection
        $allResponses.Value += $response.results

        # Check if remainingCount is greater than 0
        if ($response.remainingCount -gt 0) {
            # Recursive call
            Invoke-GetNextQuery -sessionInfo $sessionInfo -payload $payload -allResponses $allResponses
        }
    } catch {
       Write-Host $_
    }
...
function Get-RDSServerSummaryView() {
	...
	$payload = @{
        entityType = "RDSServerSummaryView"
    ...
    try {
         ...
         if ( $responseData -ne $null) {
             $payload = @{ id = $responseData.id }
             $allResponses += $responseData.results
             if ( $responseData.remainingCount -gt 0 ) {
                Invoke-GetNextQuery -sessionInfo $sessionInfo -payload $payload -allResponses ([ref]$allResponses)
             }
             Clear-QueryById $sessionInfo $responseData.id             
             # Write-Host (ConvertTo-Json -Depth 10 $allResponses)
             Write-Host $allResponses.length " rdsservers are in response."
         }
...
function Get-ApplicationInfo() {
    ...
    try {
         ...
         if ( $responseData -ne $null) {
             ...
             Clear-QueryById $sessionInfo $responseData.id             
             # Write-Host (ConvertTo-Json -Depth 10 $allResponses)
             Write-Host $allResponses.length " application info are in response."
         }
...
function Get-DesktopHealthInfo() {
	...
	$payload = @{
        entityType = "DesktopHealthInfo"
        queryEntityType = "DesktopHealthInfo"
        sortDescending = "false"
        startingOffset = "0"
    }
    $responseData = $null
    ...
    try {
         ...
         if ( $responseData -ne $null) {
             ...
             Clear-QueryById $sessionInfo $responseData.id             
             # Write-Host (ConvertTo-Json -Depth 10 $allResponses)
             Write-Host $allResponses.length " desktop health info are in response."
         }
...
function Get-FarmHealthInfo() {
    param(
        $sessionInfo
    )

	$payload = @{
        entityType = "FarmHealthInfo"
        queryEntityType = "FarmHealthInfo"
        sortDescending = "false"
        startingOffset = "0"
    }
    $responseData = $null
    ...
    try {
         ...
         if ( $responseData -ne $null) {
             ...
             Clear-QueryById $sessionInfo $responseData.id             
             # Write-Host (ConvertTo-Json -Depth 10 $allResponses)
             Write-Host $allResponses.length " farm health info are in response."
         }
...
function Get-GlobalEntitlementInfo() {
    param(
        $sessionInfo
    )

	$payload = @{
        entityType = "GlobalEntitlementInfo"
        queryEntityType = "GlobalEntitlementInfo"
        sortDescending = "false"
        startingOffset = "0"
    }
    $responseData = $null
    ...
    try {
         ...
         if ( $responseData -ne $null) {
             ...
             Clear-QueryById $sessionInfo $responseData.id             
             # Write-Host (ConvertTo-Json -Depth 10 $allResponses)
             Write-Host $allResponses.length " GlobalEntitlementInfo are in response."
         }
...
function Get-GlobalApplicationEntitlementInfo() {
    param(
        $sessionInfo
    )

	$payload = @{
        entityType = "GlobalApplicationEntitlementInfo"
        queryEntityType = "GlobalApplicationEntitlementInfo"
        sortDescending = "false"
        startingOffset = "0"
    }
    $responseData = $null
    ...
    try {
         ...
         if ( $responseData -ne $null) {
             ...
             Clear-QueryById $sessionInfo $responseData.id             
             # Write-Host (ConvertTo-Json -Depth 10 $allResponses)
             Write-Host $allResponses.length " GlobalApplicationEntitlementInfo are in response."
         }
...
function Get-QueryByEntityType() {
    param(
        $sessionInfo,
        $queryEntityType
    )
    ...
    try {
         ...
         if ( $responseData -ne $null) {
             ...
             Clear-QueryById $sessionInfo $responseData.id             
             # Write-Host (ConvertTo-Json -Depth 10 $allResponses)
             Write-Host $allResponses.length " $queryEntityType are in response."
         }
...
Export-ModuleMember -Function Get-ADDomainList, Get-ADContainerByRDN, Get-ADContainerByDomain, Add-DesktopPool, Get-DesktopPoolByName, Add-MachinesToManualDesktop, Get-RDSServerSummaryView
...
Export-ModuleMember -Function Get-DesktopHealthInfo, Get-FarmHealthInfo, Get-ApplicationInfo, Get-GlobalEntitlementInfo, Get-ApplicationEntitlementInfo, Get-QueryByEntityType
